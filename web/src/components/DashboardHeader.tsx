      ...
      <div className="grid w-full grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 md:w-auto md:grid-cols-[auto_auto]">
        ...
        <div className="min-w-0">
          ...
          <p className="text-sm leading-snug text-zinc-500">
            <span className="block md:inline">{t('app.taglineTrack')}</span>
            {' '}
...
function YearButton({ onClick, className }: { onClick: () => void; className: string }) {
  const t = useT()
  return (
    <button type="button" onClick={onClick} className={`${className} flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-zinc-800 bg-zinc-900 text-zinc-500 transition hover:border-zinc-700 hover:text-zinc-200`} aria-label={t('app.ariaYear')}>
...
function RefreshButton({ onClick, refreshing, className }: { onClick: () => void; refreshing: boolean; className: string }) {
  const t = useT()
  return (
    <button type="button" onClick={onClick} disabled={refreshing} className={`${className} flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-zinc-800 bg-zinc-900 text-zinc-500 transition hover:border-zinc-700 hover:text-zinc-200 disabled:opacity-60`} aria-label={t('app.ariaRefresh')}>
...
function SettingsButton({ onClick, className }: { onClick: () => void; className: string }) {
  const t = useT()
  return (
    <button type="button" onClick={onClick} className={`${className} flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-zinc-800 bg-zinc-900 text-zinc-500 transition hover:border-zinc-700 hover:text-zinc-200`} aria-label={t('app.ariaSettings')}>
