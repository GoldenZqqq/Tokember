...
function monthPositions(weeks: HeatmapCell[][]) {
  ...
  weeks.forEach((week, column) => {
    ...
    if (month !== lastMonth) { positions.set(column, MONTH_LABELS[month]); lastMonth = month }
  })
  return positions
}
...
function HeatmapGrid({
  ...
  const positions = monthPositions(weeks)
  return <div className="overflow-x-auto"><div className="mx-auto flex w-fit flex-col gap-1.5">
    ...
    <div className="flex gap-[3px]">
      <div className="mr-1 flex flex-col gap-[3px]">{WEEKDAY_LABELS.map((label, index) => <div
        key={index} className="flex h-2.5 w-4 items-center text-[10px] text-zinc-600">{label}</div>)}</div>
...
function HeatmapHeader({ year, metric }: { year: number; metric: YearMetric }) {
  return <div className="mb-4 flex items-center justify-between">
    <h2 className="text-sm font-medium text-zinc-400">{year} 年{YEAR_METRIC_LABELS[metric]}热力图</h2>
...
export function YearHeatmap({
  ...
  return <div className="relative rounded-xl border border-zinc-800/50 bg-zinc-900/50 p-4"
    ...
    <HeatmapGrid weeks={weeks} metric={metric} maxValue={maxValue} onHover={setHover} />
